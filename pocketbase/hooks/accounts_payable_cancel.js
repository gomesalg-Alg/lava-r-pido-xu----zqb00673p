routerAdd(
  'POST',
  '/backend/v1/accounts-payable/{id}/cancel',
  (e) => {
    ...
    if (ap.getString('status') === 'Cancelado') {
      return e.badRequestError('Esta conta já está cancelada.')
    }
    ...
    if (poId) {
      ...
      try {
        ...
        return e.badRequestError(String(err.message || err))
      }

      return e.json(200, { success: true, action: 'deleted' })
    } else {
      ap.set('status', 'Cancelado')
      $app.save(ap)
      return e.json(200, { success: true, action: 'cancelled' })
    }
  },
  $apis.requireAuth(),
)
